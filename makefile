...
CXX := g++
CXXLIBS := `pkg-config --cflags gtkmm-3.0 opencv bullet`
LDLIBS := `pkg-config --libs gtkmm-3.0 opencv bullet` -lGL -lGLEW -lsfml-system -lsfml-window -lsfml-graphics -lSOIL -lassimp
...
$(BIN_DIR)/d223: $(OBJ_FILES) | $(BIN_DIR)
	$(CXX) -o $@ $^ $(LDFLAGS)
